...
public class MMI1x2 extends AbstractElement {
	...
	public void setPorts(){
		...
		Position hVec = temp.getPort1().connect().getNormalVec().resize(lx_um) ;
		Position vVec = temp.getPort1().connect().getEdgeVec().resize(ly_um) ;
		Position vec12 = hVec.translateXY(vVec.scale(-1)) ;
		Position vec23 = vVec.resize(2*ly_um) ;
		ThreePortObject mmi = new ThreePortObject(portNumber, objectPort, new Position[] {vec12, vec23}) ;
		port1 = mmi.getPort1() ;
		port2 = mmi.getPort2() ;
		port3 = mmi.getPort3() ;
